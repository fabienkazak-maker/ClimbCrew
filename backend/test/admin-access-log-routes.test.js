...
const [serverSource, logRoutesSource] = await Promise.all([
...
test("server.js délègue la consultation des logs administrateur", () => {
  ...
  assert.match(serverSource, /app\.post\("\/admin\/auth\/users\/:id\/approve", requireAuth, requireAdmin, legacyReplacedRoute\)/);
});
