...
const [serverSource, authSessionSource] = await Promise.all([
...
test("server.js délègue les opérations d'une session déjà authentifiée", () => {
  assert.match(serverSource, /app\.post\("\/auth\/login", authRateLimit, legacyReplacedRoute\)/);
  assert.match(serverSource, /installAuthSessionRoutes\(app, \{/);
  assert.match(serverSource, /sessionDurationMs: SESSION_DURATION_MS/);
  assert.match(serverSource, /app\.post\("\/auth\/request-access", authRateLimit, legacyReplacedRoute\)/);
