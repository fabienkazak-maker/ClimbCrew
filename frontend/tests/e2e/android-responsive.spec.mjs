...
test("l'application est directement disponible sans vidéo d'introduction", async ({ page }) => {
  const pageErrors = [];
  page.on("pageerror", (error) => pageErrors.push(error.message));

  await page.goto("/", { waitUntil: "domcontentloaded" });
  await expect(page.locator("#root")).toBeVisible();
  await expect(page.locator(".startup-video")).toHaveCount(0);
  await expect(page.locator(".startup-video-app")).toHaveCount(0);
  ...
  expect(interactiveCount, "L'application sans intro doit exposer au moins un contrôle interactif").toBeGreaterThan(0);
  expect(pageErrors, `Erreurs JavaScript au chargement: ${pageErrors.join(" | ")}`).toEqual([]);
});
